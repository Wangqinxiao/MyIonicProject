...
import { ExpressionReviewPage } from '../review/review';

@Component({
  selector: 'page-expression-test',
  templateUrl: 'test.html'
})
export class ExpressionTestPage {
  ...
  next(item){
    if(this.test === 'sentenceTest'){
      this.navCtrl.push(ExpressionReviewPage, {
        item: item
      });
    }else{
      this.test = 'sentenceTest';
    }
  };
}
